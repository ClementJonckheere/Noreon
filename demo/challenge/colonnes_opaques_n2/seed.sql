-- =============================================================================
-- Noreon CHALLENGE · « Colonnes opaques — Niveau 2 »
-- Base source : noreon_demo_challenge_colonnes_opaques_n2
--
-- Question :  « Pourquoi le chiffre d'affaires baisse-t-il depuis 4 mois ? »
--
-- MÊME HISTOIRE que le scénario retail (PACA porte ~97 % de la baisse), colonnes
-- opaques ET — c'est le durcissement du Niveau 2 — AUCUNE FK DÉCLARÉE. Le moteur
-- ne peut plus s'appuyer ni sur les NOMS ni sur les métadonnées de relation. Pour
-- atteindre la région (qui vit sur t_s), il doit INFÉRER la relation
-- col_002 → t_s.k0 PAR RECOUVREMENT DE VALEURS (les valeurs de col_002 sont
-- incluses dans les clés de t_s).
--
--   • MESURE : profil (variable numérique continue, ni clé ni FK) ;
--   • RELATIONS : recouvrement de valeurs (P-05) ;
--   • CAUSE : par la VALEUR du segment (« Provence-Alpes-Côte d'Azur »).
--
...
CREATE TABLE t_o (            -- « orders »
    ...
    col_001 integer,   -- FK NON déclarée → à inférer par recouvrement de valeurs
    col_002 integer,   -- FK NON déclarée → à inférer par recouvrement de valeurs
    col_z   date,
