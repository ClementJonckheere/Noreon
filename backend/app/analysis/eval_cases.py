...
CASES: list[EvalCase] = [
    ...
    _c("rfm_full",
       ...
       ("segmentation", "distribution"), accept=("affinity", "attribution"),
       dom="retail", min_goals=3,
       edges=(("distribution", "segmentation"),), unresolved=frozenset({"dimension"}),
       note="RFM : segment + ventilation produits/segment (distribution) + tranche d'âge"),
    _c("rfm_no_acronym",
       "Segmente mes clients selon leur valeur et regarde quels types de produits chaque segment achète",
       ("segmentation", "distribution"), accept=("affinity", "attribution"),
       dom="retail", min_goals=2, edges=(("distribution", "segmentation"),)),
